...
@section('content_header')
    ...
    <div class="row">
        <h1>servicio</h1> 
        <a href="{{url('/servicio/create')}}" class="btn btn-primary">Nuevo</a>
    </div>
    ...
    <x-adminlte-datatable id="table1" :heads="$heads" :config="$config">
        @foreach($servicios as $servicio)
            <tr>
                ...
                <td>{{ $servicio->grupo_ocupa}}</td>
                <td>{{ $servicio->grado}}</td>
                <td>{{ $servicio->remuneracion}}</td>
                
                <td>
                    <div class="row">
                    <a href="{{ url('/servicio/'.$servicio->id.'/edit')}}" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar"><i class="fa fa-lg fa-fw fa-pen"></i></a>
